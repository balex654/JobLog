    ...
    useEffect(() => {
        const init = async () => {
            ...
            for (i = 0; i < gpsPoints.length - 2; i++) {
                ...
                const speed = GetVelocityByUnit(cur.speed, unit);
                const altitude = GetShortLengthValueByUnit(cur.altitude, unit);
                const distance = GetLengthValueByUnit(GetHorizontalDistance(cur, next) * 0.001, unit);
