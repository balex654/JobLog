export const DistanceYTD = `
    SELECT SUM(total_distance) * 1000 as distance_ytd FROM (WITH trip AS (
        SELECT 
            user_id,
            activity_id,
            start_date,
    ...
    FROM trip
    WHERE user_id = ? and date_part('year', start_date) = date_part('year', CURRENT_DATE)
